...
export default function WorkerDashboardPage() {
  ...
  const [selectedRes, setSelectedRes] = useState<WorkerReservation | null>(
    null,
  );
  ...
  const { data, loading, error, refetch } = useApiPath<{
    data: WorkerReservationsResponse;
  ...
  const filteredList = useMemo(
    () =>
      reservations.filter(
        (res) => res.booking_date === selectedDate && res.status === activeTab,
      ),
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button
              onClick={() =>
                changeDate(
                  viewMode === "calendar" && calendarMode === "week" ? -7 : -1,
                )
              }
                    ...
                    <div key={date} className="flex-1 text-center">
                      ...
                      <div
                        className={cn(
                          ...
                          isToday ? "text-ink" : "text-graphite",
                        )}
                      >
                        {d.getDate()}
                      </div>
            ...
            <div className="relative flex">
              ...
              <div className="absolute inset-0 ml-14 pointer-events-none">
                {Array.from({ length: 12 }).map((_, i) => (
                  ...
                  >
                    <div
                      className="absolute w-full border-t border-line border-dashed opacity-50"
              ...
              <div
                className={cn(
                  "relative flex-1",
                  calendarMode === "week" ? "flex" : "block ml-2",
                )}
              >
                {(calendarMode === "week" ? weekDates : [selectedDate]).map(
                  (colDate) => (
                    <div
                      key={colDate}
                    ...
                    >
                      {calendarReservations
                        ...
                        .map((res) => {
                          const top =
                            (getMinutesSince10(res.start_time) / 60) *
                            HOUR_HEIGHT;
                          ...
                          return (
                            <div
                              ...
                              className={cn(
                                ...
                                res.status === "BOOKED" &&
                                  "bg-mist border-ink/20 text-ink",
                                res.status === "COMPLETED" &&
                                  "bg-ink border-ink text-paper",
                                res.status === "CANCELLED" &&
                                  "bg-paper border-line text-smoke opacity-70 line-through",
                              )}
                              style={{ top, height }}
                            >
            ...
            <div className="no-scrollbar flex-1 overflow-y-auto px-6 pt-6 pb-6">
              <div className="mt-6">
                ...
                <div className="mt-6 space-y-5 border-t border-line pt-5">
                  <div>
                    <div className="text-[11px] font-bold tracking-widest text-graphite uppercase mb-1">
                      Customer
                    </div>
                  ...
                  <div>
                    ...
                    <div className="text-[15px] font-semibold text-ink">
                      {selectedRes.service_names}
                    </div>
                    <div className="text-[13px] text-graphite mt-0.5 ml-6">
                      {formatPrice(Number(selectedRes.service_total))}
                    </div>
                  </div>
                </div>
              </div>
