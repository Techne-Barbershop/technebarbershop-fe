...
import type { Staff } from "@/lib/types/admin";
...
export default function CapsterPerformancePage() {
  const { id } = useParams();
  ...
  const fetchPerformance = async () => {
    ...
    try {
      ...
      const res = await api<{ data: PerformanceResponse }>(`/api/admin/staff/${id}/performance`, {
        query
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-black">Performa Capster</h1>
            <p className="text-sm text-gray-500">Lihat statistik keberhasilan dan riwayat reservasi staf ini.</p>
          </div>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-5 rounded-lg border border-gray-200 bg-white p-5">
            {data.staff.image_url ? (
              <img src={data.staff.image_url} alt={data.staff.name} className="h-16 w-16 rounded-full object-cover border border-gray-200" />
            ) : (
              <Avatar name={data.staff.name} size="lg" className="h-16 w-16 text-xl" />
            )}
            <div className="flex-1">
              <h2 className="text-xl font-bold text-black">{data.staff.name}</h2>
              <p className="text-sm font-semibold text-gray-500 uppercase">{data.staff.role}</p>
              <div className="mt-2 flex flex-wrap gap-x-6 gap-y-2 text-sm text-gray-600">
                <span className="flex items-center gap-1.5"><Icon name="phone" className="h-4 w-4 text-gray-400" /> {data.staff.phone || "-"}</span>
                <span className="flex items-center gap-1.5"><Icon name="mail" className="h-4 w-4 text-gray-400" /> {data.staff.email || "-"}</span>
              </div>
            </div>
          </div>
                    ...
                    data.reservations.map((res) => (
                      <tr key={res.reservation_id} className="hover:bg-gray-50">
                        ...
                        <td className="px-5 py-3">
                          <p className="font-semibold text-black">{res.customer_name || "-"}</p>
                        ...
                        <td className="px-5 py-3">
                          ...
                          <p className="text-xs text-gray-500">{res.start_time}</p>
                        </td>
