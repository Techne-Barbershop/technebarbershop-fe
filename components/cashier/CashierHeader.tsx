...
const TITLES: Record<string, string> = {
  "/cashier": "Daftar Reservasi",
  "/cashier/order": "Tambah Order",
};
...
export default function CashierHeader({
  ...
  // Find matching title, allowing for subpaths
  let title = "Kasir";
  if (pathname.startsWith("/cashier/order")) {
    title = "Tambah Order";
    ...
    <header className="sticky top-0 z-30 border-b border-gray-200 bg-white">
      <div className="flex h-16 items-center gap-3 px-4 md:gap-4 md:px-6 lg:px-8">
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          <Avatar name={user?.name ?? "Kasir"} size="sm" />
        </div>
      </div>
    </header>
  );
